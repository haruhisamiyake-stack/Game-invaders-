...
// 裏面（税務調査）専用の背景セリフ（これだけを使う）
const URA_PHRASES = [
  'この経費、説明できますか？','誰と、どこで、何のために？','元帳を見せてください','通帳も確認します','反面調査を開始します',
  '前年と比べて増えていますね','この入金は何ですか？','原始資料はありますか？','個人口座も見せてください','その処理、根拠はありますか？',
  '売上はすべて計上されていますか？','現金売上の記録を確認します','この売上、計上時期は正しいですか？','請求書と入金額が一致しませんね','売掛金の残高を説明してください',
  '期末前後の取引を確認します','売上の抜けはありませんか？','この値引きの理由は何ですか？','キャンセルの記録を見せてください','レジデータと帳簿を照合します',
  '現金出納帳をお願いします','金庫の中を確認します','仮払金が残っていますね','立替金の精算はいつですか？','この支払いの相手先はどこですか？',
  '領収書の宛名が空欄ですね','この領収書、日付がありませんね','同じ金額が何度も出ていますね','これは事業に必要な支出ですか？','私的な支出が混ざっていませんか？',
  '家事按分の計算根拠を教えてください','交際費の相手方を確認します','会議費の参加者は誰ですか？','出張の行程表はありますか？','旅費規程を見せてください',
  'この外注費、実態はありますか？','給与ではなく外注費とした理由は？','業務委託契約書はありますか？','架空経費ではありませんね？','重複計上になっていませんか？',
  '棚卸表を見せてください','在庫の数が合いませんね','廃棄した記録はありますか？','棚卸資産の評価方法は？','仕入先の請求書を確認します',
  '仕入と売上の対応を説明してください','この在庫、どこに保管していますか？','期末仕入が増えていますね','返品の処理を確認します','サンプル品の管理はどうしていますか？',
  '固定資産台帳を見せてください','これは修繕費ですか、資産計上ですか？','減価償却の計算を確認します','除却した証拠はありますか？','この車両の使用実態を教えてください',
  '自宅兼事務所の図面はありますか？','リース契約書を見せてください','少額資産とした判定根拠は？','このソフトウェアの利用目的は？','私用分を区分していますか？',
  '給与台帳を見せてください','源泉徴収は正しく行っていますか？','この賞与、事前に届け出ていますか？','役員報酬を変更した理由は？','家族従業員の勤務実態はありますか？',
  '出勤記録を確認します','現金手渡しの記録はありますか？','社宅の使用状況を教えてください','福利厚生費の対象者は全員ですか？','退職金の算定根拠を見せてください',
  '消費税の区分を確認します','課税売上割合の計算は正しいですか？','非課税取引とした根拠は？','仕入税額控除の要件を満たしていますか？','インボイスの登録番号を確認します',
  '簡易課税の事業区分は適切ですか？','輸出免税の証明書類はありますか？','課税時期がずれていませんか？','対価の返還処理を確認します','帳簿の保存要件を満たしていますか？',
  'データの保存場所を教えてください','電子帳簿保存法への対応は？','メールのやり取りも確認します','契約書の原本はありますか？','このファイルの更新履歴を見せてください',
  '削除したデータはありませんか？','バックアップを確認できますか？','会計ソフトの入力履歴を見せてください','修正仕訳の理由を説明してください','税理士とのやり取りを確認します',
  '取引先に確認してもよろしいですね？','事実関係を整理しましょう','回答は記録させていただきます','後ほど資料を追加でお願いします','この場で確認できない理由は何ですか？',
  '説明と帳簿が一致していません','もう一度、順番に説明してください','修正申告をご検討ください','調査結果は改めてお伝えします','まだ確認は終わっていません'
];
function pickPhrase(){ const a = ura ? URA_PHRASES : WALL_PHRASES; return a[Math.floor(Math.random()*a.length)]; }
...
function bossDown(){
  ...
  if(b.phase === 1){
    ...
    bgmSet('kanta', true);   // 第三形態＝かんた専用BGMへ切替
...
const BGM = {
  ...
  kanta:  { file: 'assets/kanta-theme.mp3',    vol: .5  }   // 表ラスボス第三形態（かんた）
};
...
function fireBeam(p, stage){
  const st2 = stage === 2;
  const pw = Math.min(100, p);   // ビームの太さ・持続はゲージ一周ぶんで頭打ち
  const life = Math.round((22 + pw * .22) * (st2 ? 1.15 : 1));
  beam = { x: player.x, w: (26 + pw * 1.25) * (st2 ? 1.7 : 1),
           power: pw * (st2 ? 1.6 : 1), life: life, maxlife: life, acc: 0,
           stage: stage, second: (stage === 1 && p >= STAGE2_TH) };   // 二周溜め切りで二段目を予約
  flash = st2 ? 18 : 6; shake = Math.round((st2 ? 20 : 6) + pw * .1);
  if(st2){
...
function update(){
  ...
  if(held && (charge > 0 || ki >= 15)){
    ...
    if(before < STAGE2_TH && charge >= STAGE2_TH){   // 二段撃ち解禁（二周溜め切り）の合図
      beep(1300, .14, 'square', .06); beep(1750, .14, 'sine', .05); flash = Math.max(flash, 7);
    }
    if(frame % 5 === 0) beep(240 + (charge % 100)*7, .05, 'triangle', .025);
  ...
  if(!bossObj){
    if(++bgPhraseT >= 220){ bgPhraseT = 0; bgPhrase = pickPhrase(); }
  }
...
/* ---------- 表面：急降下・回り込みする敵 ---------- */
function maybeStartDive(live){
  const diving = live.filter(e => e.diving).length;
  const maxDivers = 2 + Math.floor(wave / 4);
  if(diving >= maxDivers) return;
  const rate = 0.006 + wave * 0.0015 + (midDone ? 0.004 : 0);   // 面が進む・中ボス後ほど頻繁
  if(Math.random() >= rate) return;
  const cand = live.filter(e => e.alive && !e.diving && !e.float);
  if(!cand.length) return;
  const e = cand[Math.floor(Math.random() * cand.length)];
  e.diving = true; e.dt = 0; e.sx = e.x; e.sy = e.y;
  e.tx = Math.max(24, Math.min(W - 24, player.x + (Math.random() * 80 - 40)));   // 自機付近へ回り込む
  beep(300, .06, 'sawtooth', .03);
}
function updateDivers(){
  ...
  for(const e of enemies){
    ...
    e.dt++;
    const p = e.dt / DUR, s = Math.sin(p * Math.PI);
    const depth = Math.max(100, H - 80 - e.sy);
    e.y = e.sy + s * depth;                    // 下へ弧を描いて戻る
    e.x = e.sx + (e.tx - e.sx) * s;            // 自機側へ回り込み、戻る
    // 自機との体当たり
    if(player.inv <= 0 && Math.abs(e.x - player.x) < 15 && Math.abs(e.y - player.y) < 13){
      if(player.shield > 0){ player.shield--; player.inv = 60; shake = 8; beep(300, .15, 'triangle', .05); }
      else { lives--; player.inv = 90; shake = 14; beep(120, .3, 'sawtooth', .07); if(lives <= 0){ gameOver(); return; } }
      e.diving = false; e.dt = 0; e.x = e.sx; e.y = e.sy;
    }
    if(e.dt >= DUR){ e.diving = false; e.x = e.sx; e.y = e.sy; }   // 隊列へ帰還
  }
}
...
function updateSwarm(){
  ...
  // 表面：時々、隊列から離れて急降下・回り込みする敵（回避の駆け引き）
  if(!ura){ maybeStartDive(live); }
  updateDivers();

  // 隊列移動（マーチ）。急降下中・フロートは隊列から除外
  const marchers = live.filter(e => !e.float && !e.diving);
  if(marchers.length){
...
function spawnShochoAdds(b){
  b.addsSpawned = true;
  shake = 16; flash = 12;
  bigMsg('かんたに手を出すな！', 130);   // 所長の決めゼリフ（かんた＝人名）をデカデカと
...
function drawCharge(){
  const lap2 = charge >= 100;             // 二周目に入っているか
  const ready2 = charge >= STAGE2_TH;     // 二段撃ち解禁（二周溜め切り）
  const frac = lap2 ? Math.min(1, (charge - 100) / 100) : charge / 100;
  const r = 15 + Math.min(100, charge) * .16, cx = player.x, cy = player.y - 2;
  const TWO_PI = Math.PI * 2, top = -Math.PI / 2;
  ctx.save();
  // 内側の充填
  ctx.globalAlpha = .25 + .2*Math.sin(frame/4);
  ctx.fillStyle = ready2 ? '#ff5252' : (lap2 ? '#e0b83a' : '#d8b45c');
  ctx.beginPath(); ctx.arc(cx, cy, r*.8, 0, TWO_PI); ctx.fill();
  ctx.globalAlpha = 1;
  // 一周目の完了リング（二周目では下地として全周を朱で表示）
  if(lap2){
    ctx.strokeStyle = 'rgba(192,57,43,.55)'; ctx.lineWidth = 2.5;
    ctx.beginPath(); ctx.arc(cx, cy, r, 0, TWO_PI); ctx.stroke();
  }
  // 現在の周の進捗リング（一周目＝朱／二周目＝金）
  ctx.strokeStyle = lap2 ? '#ffd23f' : '#c0392b'; ctx.lineWidth = lap2 ? 4 : 2.5;
  ctx.beginPath(); ctx.arc(cx, cy, r, top, top + TWO_PI * frac); ctx.stroke();
  // 周回表示
  ctx.fillStyle = lap2 ? '#ffd23f' : 'rgba(237,228,211,.85)'; ctx.font = 'bold 9px system-ui,sans-serif';
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.fillText(lap2 ? '2周目' : '1周目', cx, cy - 1);
  // 二段撃ち解禁：脈動リング＋「二段撃ちOK！」
  if(ready2){
    const pulse = .5 + .5*Math.sin(frame/4);
    ctx.globalAlpha = .35 + .45*pulse; ctx.strokeStyle = '#ffd23f'; ctx.lineWidth = 2.5;
    ctx.beginPath(); ctx.arc(cx, cy, r + 5 + pulse*4, 0, TWO_PI); ctx.stroke();
    ctx.globalAlpha = 1;
